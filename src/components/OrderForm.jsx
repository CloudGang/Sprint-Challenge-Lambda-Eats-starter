...
export default function OrderForm({ order, setOrder }) {
  ...
  const initialState = {
    ...
    size: '',
    crust: [
      { name: 'Brooklyn Style', id: 'brooklyn-style', isChecked: false },
      { name: 'Hand Tossed', id: 'hand-tossed', isChecked: false },
      { name: 'Crunchy Thin Crust', id: 'thin-crust', isChecked: false },
    ],
    sauces: [
      { name: 'Robust Tomato Sauce', id: 'tomato-sauce', isChecked: false },
        ...
        {formState.crust.map(crust => {
          return (
            <StyledLabel htlmFor={crust.id} check>
              <Input type="radio" name="crust" id={crust.id} onChange={e => handleChange(e)} value={crust.isChecked} checked={crust.isChecked} />
              {crust.name}
            </StyledLabel>
          )
        })}
